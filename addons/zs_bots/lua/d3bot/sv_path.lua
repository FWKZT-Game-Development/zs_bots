	...
	while true do
		...
		for linkedNode, link in pairs(node.LinkByLinkedNode) do
			...
			if able and not blocked and not (link.Params.Direction == "Forward" and link.Nodes[2] == node) and not (link.Params.Direction == "Backward" and link.Nodes[1] == node) then
				local linkedNodePathCost = minimalPathCostByNode[node] + math_max(node.Pos:Distance(linkedNode.Pos) + (linkedNode.Params.Cost or 0) + (link.Params.Cost or 0) + (pathCostFunction and pathCostFunction(node, linkedNode, link) or 0), 0) -- Prevent negative change of the link costs, otherwise it will get stuck decreasing forever
...
function D3bot.GetEscapeMeshPathOrNil(startNode, iterations, pathCostFunction, heuristicCostFunction, abilities)
	...
	while true do
		...
		for linkedNode, link in pairs(node.LinkByLinkedNode) do
			...
			if able and not blocked and not (link.Params.Direction == "Forward" and link.Nodes[2] == node) and not (link.Params.Direction == "Backward" and link.Nodes[1] == node) then
				local linkedNodePathCost = minimalPathCostByNode[node] + math_max((linkedNode.Params.Cost or 0) + (link.Params.Cost or 0) + (pathCostFunction and pathCostFunction(node, linkedNode, link) or 0), 0) -- Prevent negative change of the link costs, otherwise it will get stuck decreasing forever
				if linkedNodePathCost < (minimalPathCostByNode[linkedNode] or math.huge) then
					entranceByNode[linkedNode] = node
					minimalPathCostByNode[linkedNode] = linkedNodePathCost
...
function D3bot.GetBestMeshPathOrNil( startArea, endArea, pathCostFunction, heuristicCostFunction, abilities )
	...
	local nodeReach = 0 -- The pathfinding can only see x amount of areas ahead, this is to prevent lag from searching for an area it cannot see and creating an infinite loop
	
	while not startArea:IsOpenListEmpty() do
		local current = startArea:PopOpenList()
		if not current or nodeReach >= D3bot.NavSearchDist then return end
		...
		for _, linkedArea in ipairs( current:GetAdjacentAreas() ) do
			...
			if able and not blocked then
				local newCostSoFar = current:GetCostSoFar() + math_max( current:GetCenter():Distance( linkedArea:GetCenter() ) + ( ( linkedAreaData.Params.Cost and current:GetCostSoFar() / 4 ) or 0 ) + ( ( linkData.Params.Cost and current:GetCostSoFar() / 4 ) or 0 ) + ( pathCostFunction and pathCostFunction( current, linkedArea, link ) or 0 ), 0 )
...
function D3bot.GetEscapeMeshPathOrNil( startArea, iterations, pathCostFunction, heuristicCostFunction, abilities )
